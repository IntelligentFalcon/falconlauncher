...
export function MirrorSettings() {
  ...
  const processJsonString = async (jsonText: string) => {
    try {
      ...
      setImportError(
          typeof err === "string"
              ? err
              : t("mirrorSettings.invalidJsonError") // <-- Translated
      );
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (
      ...
      setImportError(t("mirrorSettings.invalidFileTypeError")); // <-- Translated
    }
  };
  ...
  return (
      <LoadingSwap className="h-full w-full" isLoading={isQueriesLoading}>
        <div className="max-w-2xl space-y-6">
          ...
          <section
              aria-label={t("mirrorSettings.dropzoneAriaLabel")} // <-- Translated
          ...
          >
            ...
            <div className="font-medium text-foreground text-xs">
              {t("mirrorSettings.dropzoneTitle")}
            </div>
            <div className="text-[10px]">
              {t("mirrorSettings.dropzoneSubtitle")}
            </div>
