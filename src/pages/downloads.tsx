...
export default function InstallerWizard() {
  ...
  const getBackendCommand = () => {
    switch (activeLoader) {
      case 'forge': return 'get_forge_versions';
      case 'fabric': return 'get_fabric_versions';
      case 'vanilla':
      default: return 'get_vanilla_versions';
    }
  };
  ...
  useEffect(() => {
    ...
    const validData = data.filter((cat: any) => cat.versions && cat.versions.length > 0);
    if (validData.length === 0) return;

    const hasMajor = validData.some((v: any) => v.name === activeMajorVersion);
    ...
    const versions = validData.find((v: any) => selectedMajor === v.name)?.versions;
    const versionMatch = versions?.find((v: any) => v.id === activeVersion?.id);
              ...
              <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="mb-8 text-center space-y-2">
                  <div className="mx-auto bg-primary/20 p-3 rounded-full w-fit mb-4">
                    <HugeiconsIcon icon={PackageIcon} size={32} className="text-primary" />
                  </div>
                  <h2 className="text-2xl font-bold tracking-tight text-foreground">Choose Environment</h2>
                  <p className="text-muted-foreground text-sm">Select the mod loader you want to install.</p>
                </div>
          ...
          {step === 2 && (
              <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="flex items-center mb-6">
                  ...
                  <h2 className="text-xl font-bold tracking-tight ml-2">Configure Instance</h2>
                  <div className="ml-auto px-3 py-1 bg-primary/10 text-primary text-xs font-bold uppercase rounded-full tracking-wider">
                    {activeLoader}
                  </div>
                </div>

                <LoadingSwap isLoading={isLoading} className="flex-1 flex flex-col min-h-0">
                  <div className="space-y-6 overflow-y-auto pr-2 pb-4 scrollbar-thin flex-1">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {data?.filter((cat: any) => cat.versions && cat.versions.length > 0).map((v: any) => {
                    ...
                    <div className="space-y-2">
                      ...
                      <Combobox
                          items={data?.find((v: any) => activeMajorVersion === v.name)?.versions || []}
                          autoHighlight
                          value={activeVersion}
          ...
          {step === 3 && (
              <div className="flex flex-col h-full items-center justify-center animate-in zoom-in-95 duration-300">
                <div className="mb-8 p-4 bg-secondary/40 rounded-full border border-border/40">
                  {isDone ? (
                      <HugeiconsIcon icon={CheckmarkBadge01Icon} size={48} className="text-emerald-500 animate-in spin-in-12 duration-500" />
                  ) : (
                      <HugeiconsIcon icon={Settings01Icon} size={48} className="text-primary animate-spin" />
                  )}
                </div>
                ...
                <p className="text-muted-foreground text-sm max-w-[80%] text-center truncate mb-8">
                  {isDone ? `Successfully installed ${instanceName || activeVersion?.id}` : statusText}
                </p>
