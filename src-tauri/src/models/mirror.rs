...
impl Mirror {
    ...
    pub async fn is_connected(&self, cfg: &Config) -> bool {
        ...
        for url in self.maps.values() {
            ...
            let Ok(_) = client.head(url).send().await else {
                info!("Failed to connect to the mirror's url");
                t = false;
                break;
            };
        }
    ...
    pub fn write(&self) -> Result<(), AppError> {
        let content =
            serde_json::to_string(&self).map_err(|x| AppError::FileWriteFailed(x.to_string()))?;
        fs::write(
...
pub fn list_mirrors() -> Result<Vec<Mirror>, AppError> {
    ...
    let mut vec = Vec::new();
    let Ok(read_dir) = mirrors_dir.read_dir() else {
        info!(
            "Failed to read mirrors directory! {}. returning official mirror only.",
            mirrors_dir.display()
        );
        mojang_mirror().write()?;
    ...
    if vec
        .iter()
        .find(|x| x.name == mojang_mirror().name)
        .is_none()
    {
...
pub fn mirror_from(name: &String) -> Mirror {
    let Ok(mirrors) = list_mirrors() else {
        info!("Failed to get mirrors available.");
        return mojang_mirror()
    };
    if mirrors.iter().filter(|x| x.name == *name).count() == 0 {
        mojang_mirror()
    } else {
        mirrors.iter()
            .find(|x| x.name == *name)
            .unwrap_or(&mojang_mirror())
            .clone()
    }
}
